...
class TaskReminderLinkDaoDrift extends DatabaseAccessor<OrganizerDriftDB>
  ...
  Future<int> addTaskReminder(Insertable<TaskReminderTableDriftG> taskReminder) =>
      into(taskReminderLinkTableDrift).insert(taskReminder);

  Future<bool> updateTaskReminder(Insertable<TaskReminderTableDriftG> taskReminder) =>
      update(taskReminderLinkTableDrift).replace(taskReminder);

  Future<int> deleteTaskReminderByTaskId(int taskId) async {
    return (delete(taskReminderLinkTableDrift)..where((tbl) => tbl.taskId.equals(taskId))).go();
  }

  Future<int> deleteTaskReminder(int taskId, int reminderId) async {
    return (delete(taskReminderLinkTableDrift)
          ..where((tbl) => tbl.taskId.equals(taskId) & tbl.reminderId.equals(reminderId)))
        .go();
  }
  ...
  Future<Set<int>> getReminderIdsByTaskId(int taskId) async {
    //todo cg
    final result =
        await (select(taskReminderLinkTableDrift)..where((tbl) => tbl.taskId.equals(taskId))).get();
